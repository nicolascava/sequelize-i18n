...
class SequelizeI18N {
  ...
  constructor(sequelize, options) {
    ...
    if (
      this.baseOptions.defaultLanguage &&
      this.baseOptions.languages.indexOf(this.baseOptions.defaultLanguage) == -1
    ) {
    ...
    if (instance && instance.dataValues && i18nModel.model) {
      Object.keys(instance.dataValues).forEach((prop) => {
        if (prop in i18nModel.model)
          i18nOptions[prop] = instance.dataValues[prop];
      });
        ...
        if (i18nModel) {
          ...
          model.prototype.addI18N = function (newValues, languageID) {
            ...
            if (
              ...
              !languageID ||
              !baseOptions ||
              !_.includes(baseOptions.languages, languageID)
            )
              return;
        ...
        if ("i18n" in mutableModel[prop] && mutableModel[prop].i18n === true) {
          ...
          if (
            ...
            baseOptions.indexes.push({
              unique: true,
              fields:
                mutableOptions.paranoid == true
                  ? ["language_id", "deletedAt", prop]
                  : ["language_id", prop],
            });
          }
